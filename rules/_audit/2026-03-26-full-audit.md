...
| **수정 완료** | **4건** (V2, V3, V4, V5) |
| **미수정** | **1건** (V1: P4 불변 상태, 대규모 리팩토링 필요) |
| **경고** | **1건** |
| **준수** | **11개 규칙 준수** (수정 후) |
...
- 리사이즈 후 CanvasFontMetrics/BoxRenderer 재생성 확인
...
| V1: P4 불변 상태 | critical | ⏳ 미수정 | — | 대규모 리팩토링 필요, 별도 계획 수립 권장 |

## 잔여 이슈

### V1. [critical] P4 — 불변 상태 위반 (미수정)

`src/editor.ts`의 MathEditor 클래스가 AST 노드의 children 배열을 `.splice()`로 직접 변경하고,
커서 상태를 `this.state.cursor.offset++`로 직접 변경한다. 25+ 개소에 걸쳐 있으며,
에디터 전체 상태 관리 방식의 대규모 리팩토링이 필요하므로 별도 작업으로 진행해야 한다.
