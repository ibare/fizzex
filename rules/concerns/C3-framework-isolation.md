...
## When to Apply
core 모듈(box/, latex/, types.ts, editor.ts, analyzer/, cas/, utils/, canvas/, compute/, semantic/)을 수정할 때.
...
## MUST NOT
- `box/`, `latex/`, `types.ts`, `editor.ts`, `analyzer/`, `cas/`, `utils/`, `canvas/`, `compute/`, `semantic/`에서 `react`, `react-dom`을 import하지 않는다
  - `compute/`, `semantic/`은 패키지 subpath 진입점이라 더 강하다 — 프레임워크뿐 아니라 어떤 외부 패키지도 import하지 않는다 (C6 참조)
