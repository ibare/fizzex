...
## When to Apply
Box 타입 추가, 레이아웃 로직 수정, 렌더링 파이프라인 변경 시.

## MUST
- Box의 y 좌표는 항상 baseline 위치를 의미한다 (top-left가 아님)
- height는 baseline 위 픽셀, depth는 baseline 아래 픽셀이다
- Canvas 크기 변경 후 CanvasFontMetrics와 BoxRenderer를 재생성한다 (context state가 리셋되므로)
- HiDPI 처리: `canvas.width = cssWidth * dpr; ctx.scale(dpr, dpr)` 패턴을 따른다
