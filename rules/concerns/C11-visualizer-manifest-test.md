...
- 새 viz spec 에 칩을 띄우려면 그 viz 를 소유할 **형식**(`src/analyzer/semantic/data/form/**`)에 등재하고 `examples`도 함께 채운다 — 형식에 없으면 칩이 뜨지 않는다 (C8 참조)
- `manifest.json`에 등록된 모든 viz id는 대응 `<id>/spec.json`이 존재해야 한다
- `<id>/spec.json`이 존재하는 모든 디렉터리는 `manifest.json`에 등록되어야 한다 (고아 spec 금지)
- `spec.json`의 `catalog` 참조(`"<category>/<id>"`)는 실재하는 카탈로그 항목을 가리켜야 한다 — `compileSpec`이 미지 참조에 throw 한다
