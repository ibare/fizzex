 ...
 *
 * 수평 점선 0.5 / 0.25 / 0.125 + 각각에서 수직선 교점 → 반감기 일정성 강조.
 * correct(반투명 점선) vs current(실선) 이중.
 */
...
export function drawAbstract(args: AbstractArgs): void {
  ...
  // 현재 t 지점
  if (t >= 0 && t <= tMax) {
    const ratio = Math.max(0, Math.min(1, Math.exp(r * t)));
    const cx = tToX(t);
