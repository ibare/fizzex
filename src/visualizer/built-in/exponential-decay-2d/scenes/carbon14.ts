/**
 * ⚛️ 탄소-14 장면 — 10×10 원자 입자 그리드 + 유물 실루엣
 *
 * 활성 개수 = Math.round(100 * ratio). 비활성 전환은 고정 순서 (시드 기반).
 */

import { hexAlpha, type SceneDrawArgs } from '../utils';

// 고정 순서 (0~99 번호를 무작위처럼 섞은 시퀀스). 결정론적.
const ORDER: number[] = (() => {
  const arr = Array.from({ length: 100 }, (_, i) => i);
  // 단순 Fisher-Yates with fixed LCG
  let seed = 12345;
...
export function drawCarbon14(args: SceneDrawArgs): void {
  const { ctx, x, y, w, h, ratio, ratio_std, isStandard, color, isDark, alpha = 1 } = args;
  ctx.save();
  ...
  for (let i = 0; i < 100; i++) {
    ...
    const rank = ORDER.indexOf(i); // 이 번호의 감소 순위
    // rank가 active 이상이면 비활성
