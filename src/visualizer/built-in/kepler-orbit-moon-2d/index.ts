 ...
 * 케플러 제3법칙 — 달 2D 궤도.
 *
 * T² = (4π² / GM) · a³. 기본 a=384,400 km(약 27.3일 주기).
 */

import type { FizzexVisualizer, ParameterValues, ComputeContext } from '../../types';
import { KeplerOrbitMoonRenderer } from './renderer';
...
export default (function create(): FizzexVisualizer {
  let renderer: KeplerOrbitMoonRenderer | null = null;

  return {
    id: 'kepler-orbit-moon-2d',
    name: '달 궤도',
    ...
    mount(container, options) {
      renderer = new KeplerOrbitMoonRenderer(container, options);
    },
    update(context) {
