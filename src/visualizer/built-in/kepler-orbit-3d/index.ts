/**
 * 케플러 제3법칙 — 3D 위성 궤도 Visualizer
 *
 * T^2 = (4pi^2 / GM) * a^3
 * 지구를 회전하며 보는 입체 뷰. Three.js 기반.
 */
...
const keplerOrbit3DVisualizer: FizzexVisualizer = {
  id: 'kepler-orbit-3d',
  name: '3D 궤도',
  ...
  mount(container, options) {
    (this as unknown as { _renderer: KeplerOrbit3DRenderer })._renderer =
      new KeplerOrbit3DRenderer(container, options, this.presets);
  },

  update(context) {
    const r = (this as unknown as { _renderer?: KeplerOrbit3DRenderer })._renderer;
    r?.update(context);
  },
  ...
  unmount() {
    const r = (this as unknown as { _renderer?: KeplerOrbit3DRenderer })._renderer;
    r?.destroy();
    delete (this as unknown as { _renderer?: KeplerOrbit3DRenderer })._renderer;
  },
...
export default keplerOrbit3DVisualizer;
