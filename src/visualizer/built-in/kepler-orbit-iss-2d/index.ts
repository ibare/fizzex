/**
 * 케플러 제3법칙 — ISS(국제우주정거장) 2D 궤도.
 *
 * T² = (4π² / GM) · a³. 기본 a=6,771 km(고도 ≈ 400 km, 약 90분 주기).
 */

import type { FizzexVisualizer, ParameterValues, ComputeContext } from '../../types';
import { KeplerOrbitIssRenderer } from './renderer';
...
export default (function create(): FizzexVisualizer {
  let renderer: KeplerOrbitIssRenderer | null = null;

  return {
    id: 'kepler-orbit-iss-2d',
    name: 'ISS 궤도',
    ...
    mount(container, options) {
      renderer = new KeplerOrbitIssRenderer(container, options);
    },
    update(context) {
