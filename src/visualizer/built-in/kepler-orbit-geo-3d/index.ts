/**
 * 케플러 제3법칙 — 정지궤도 3D (Three.js).
 */

import type { FizzexVisualizer, ParameterValues, ComputeContext } from '../../types';
import { KeplerOrbitGeo3DRenderer } from './renderer';
...
export default (function create(): FizzexVisualizer {
  let renderer: KeplerOrbitGeo3DRenderer | null = null;

  return {
    id: 'kepler-orbit-geo-3d',
    name: '정지궤도 3D',
    ...
    mount(container, options) {
      renderer = new KeplerOrbitGeo3DRenderer(container, options);
    },
    update(context) {
