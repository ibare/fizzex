 ...
 * 결합 표기 ↔ 화면에 남길 글자.
 *
 * 단일(`-`)·이중(`=`)·삼중(`#`) 결합이다. 아직 결합선으로 조판하지 않지만
 * 입력을 버리지는 않는다 — 버리면 `O=C=O` 가 `OCO` 가 되어 다른 화학식이 된다.
 *
 * 셋을 한 표에 두는 이유: 어느 하나만 특별히 다루면 파서에 예외 분기가 남고,
 * 실제로 그렇게 `-` 만 보존되고 나머지 둘은 사라지고 있었다.
 */
export const CHEM_BOND_MARKS: ReadonlyMap<string, string> = new Map([
  ['-', MINUS_SIGN],
  ['=', '='],
  ['#', '#'],
]);
