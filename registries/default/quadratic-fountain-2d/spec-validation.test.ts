...
describe('quadratic-fountain-2d spec.json', () => {
  it('validateSpec 통과', () => {
  ...
  it('핵심 필드 보존', () => {
    ...
    expect(parsed.id).toBe('quadratic-fountain-2d');
    expect(parsed.catalog).toBe('linear-functions/quadratic-standard');
    expect(parsed.renderer).toBe('2d');
    expect(parsed.scenes).toHaveLength(1);
    expect(parsed.scenes[0].id).toBe('fountain');
    expect(parsed.scenes[0].params).toEqual({ a: -2, b: 4, c: 0 });
  });
  ...
  it('root let에 launchX·landX·span·poolY 포함', () => {
    const parsed = validateSpec(spec);
    const letKeys = Object.keys(parsed.root.let ?? {});
    expect(letKeys).toEqual(expect.arrayContaining(['launchX', 'landX', 'span', 'poolY']));
  });
});
